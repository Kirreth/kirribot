...
def berechne_level_und_progress(counter: int) -> tuple[int, float]:
    ...
    progress: float = (counter - naechstes_level_anfang) / (
        naechstes_level_ende - naechstes_level_anfang
    )
...
def get_warns(user_id: str, guild_id: str, within_hours: Optional[int] = None) -> list[sqlite3.Row]:
    ...
    cursor = conn.cursor()
    if within_hours:
        time_limit: datetime = datetime.utcnow() - timedelta(hours=within_hours)
        cursor.execute(
...
def get_bump_top(guild_id: str, days: Optional[int] = None, limit: int = 10) -> list[sqlite3.Row]:
    conn = get_connection()
    cursor = conn.cursor()
    if days:

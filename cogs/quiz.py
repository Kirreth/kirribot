...
class Quiz(commands.Cog):
    ...
    # ------------------------------------------------------------
    @app_commands.command(name="quizadd", description="Füge eine neue Quizfrage hinzu (nur Admins).")
    ...
    async def quizadd(
        ...
        if correct not in [1, 2, 3, 4]:
            await interaction.response.send_message("❌ Die richtige Antwort muss 1–4 sein.", ephemeral=True)
            return
